...
export async function POST(request) {
  const { username, defaultPassword, newPassword } = await request.json();
  const client = await clientPromise;
  const db = client.db();
  ...
  try {
    ...
    try {
      ...
      const emailTemplate = `... your email HTML template ...`;
      ...
      console.error(
        "📨 Email sending failed after password change:",
        emailError.message
      );
      // But still continue
    }

    // 🎯 Password changed successfully regardless of email sending
    return new Response(
      JSON.stringify({ message: "Password changed successfully!" }),
      { status: 200 }
    );
